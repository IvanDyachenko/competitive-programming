...
import com.adventofcode.year2023.day05.Part1.Converters.{
  SeedToSoil,
  SoilToFertilizer,
  FertilizerToWater,
  WaterToLight,
...
object Part1 extends App {
  ...
  final case class Seeds(seeds: Set[Long]) extends AnyVal

  object Seeds {
    def apply: String => Seeds = { case s"seeds: $seeds" => Seeds(seeds.split(' ').flatMap(_.toLongOption).toSet) }
  }
  ...
  val answer = seeds.seeds.map(seed => almanac.location(seed)).min
