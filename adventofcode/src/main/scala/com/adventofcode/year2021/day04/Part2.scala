  ...
  final case class Bingo(numbers: Map[Int, Int], boards: List[Board]) {
    def score: Option[(Int, Long)] = boards
      .foldLeft[Option[(Int, Long)]](None) {
        case (loser @ Some((loserMoves, loserScore)), board) =>
          board
            .score(numbers)
            .map {
              case (boardMoves, boardScore) if boardMoves > loserMoves  => (boardMoves, boardScore)
              case (boardMoves, boardScore) if boardMoves == loserMoves => (loserMoves, loserScore min boardScore)
              case _                                                    => (loserMoves, loserScore)
            }
            .orElse(loser)
